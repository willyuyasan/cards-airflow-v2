...
def _handle_databricks_operator_execution(self, hook, log, context):
    """
    ...
    log.info('Run submitted with run_id: %s', self.run_id)
    ...
    while True:
        ...
        if run_state.is_terminal:
            if run_state.is_successful:
                ...
                _log_paths(self, context = context)            
                _print_logs(self, hook = hook, log = log)
                error_message = '{t} failed with terminal state: {s}'.format(t=self.task_id, s=run_state)
                raise AirflowException(error_message)
        else:
            log.info('%s in run state: %s', self.task_id, run_state)
            log.info('View run status, Spark UI, and logs at %s', run_page_url)
            # This is where we will call the logs part:
            # Try and gather details about logging options in this cluster    
            # Only if run_state is in running mode:              
            if run_state.life_cycle_state == 'RUNNING':
                _log_paths(self, context = context)            
                _print_logs(self, hook = hook, log = log)
            log.info('Sleeping for %s seconds.', self.polling_period_seconds)
            time.sleep(self.polling_period_seconds)

def _list_dbfs(hook, path, LIST_DBFS_ENDPOINT):
        """list_dbfs:
        ...
        try:
            return hook._do_api_call(LIST_DBFS_ENDPOINT, params)
        except AirflowException as ex:
            raise AirflowException(f"Failed to list {path}: {ex}")

def _read_dbfs(hook, path, READ_DBFS_ENDPOINT):
        """get_dbfs_file
        ...
        try:
            response = hook._do_api_call(READ_DBFS_ENDPOINT, params)
            if 'data' in response:
...
def _print_logs(self, hook, log):
        ...
        if self.log_type is not None:            
            # Retrieve Spark Logs
            log.info(f"Spark logs will be stored in {self.spark_logs}")
            spark_filelist = _list_dbfs(hook, self.spark_logs, LIST_DBFS_ENDPOINT)
            if spark_filelist is not None:
                for f in spark_filelist['files']:
                    if not f['is_dir']:
                        log.info(f['path'])
                        if 'stderr' in f['path']:
                            for line in _read_dbfs(hook, f['path'], READ_DBFS_ENDPOINT).decode('utf-8').replace('\n',"\n").split("\n"):
                                log.error(line)
                        else:
                            for line in _read_dbfs(hook, f['path'], READ_DBFS_ENDPOINT).decode('utf-8').replace('\n',"\n").split("\n"):
                                log.info(line)
            ...
            log.info(f"Executor logs will be stored in {self.exec_logs}")
            exec_filelist = _list_dbfs(hook, self.exec_logs, LIST_DBFS_ENDPOINT)
            if exec_filelist is not None:
                for app in exec_filelist['files']:
                    for executor in _list_dbfs(hook, app['path'], LIST_DBFS_ENDPOINT)['files']:
                        for f in _list_dbfs(hook, executor['path'], LIST_DBFS_ENDPOINT)['files']:
                            if not f['is_dir']:
                              log.info(f['path'])
                              if 'stderr' in f['path']:
                                  for line in _read_dbfs(hook, f['path'], READ_DBFS_ENDPOINT).decode('utf-8').replace('\n',"\n").split("\n"):
...
def _log_paths(self, context):
        ...
        if 'cluster_log_conf' in self.json['new_cluster'] and len(self.json['new_cluster']['cluster_log_conf'].keys()):
            ...
            self.spark_logs = f'{self.log_dest}/{cluster_id}/driver'
            self.exec_logs = f'{self.log_dest}/{cluster_id}/executor'

class FinServDatabricksSubmitRunOperator2(DatabricksSubmitRunOperator):
    """
    ...
    to our ``FinServDatabricksSubmitRunOperator`` through the ``json`` parameter.
    For example ::
        ...
        notebook_run = FinServDatabricksSubmitRunOperator(task_id='notebook_run', json=json)

    Another way to accomplish the same thing is to use the named parameters
    of the ``FinServDatabricksSubmitRunOperator`` directly. Note that there is exactly
    ...
    endpoint. In this method, your code would look like this: ::
        ...
        notebook_run = FinServDatabricksSubmitRunOperator(
    ...
    Currently the named parameters that ``FinServDatabricksSubmitRunOperator`` supports are
        - ``spark_jar_task``
        - ``notebook_task``
        - ``spark_python_task``
        - ``spark_submit_task``
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Creates a new ``FinServDatabricksSubmitRunOperator``.
        """
    ...
    def cluster_id(self):
        ...
        if self._cluster_id is None and self.run_id is not None:
            ...
            response = self._get_hook()._do_api_call(('GET', 'api/2.0/jobs/runs/get'), json)
...
class FinServDatabricksRunNowOperator2(DatabricksRunNowOperator):
    """
    ...
    to our ``FinServDatabricksRunNowOperator`` through the ``json`` parameter.
    For example ::
        ...
        notebook_run = FinServDatabricksRunNowOperator(task_id='notebook_run', json=json)

    Another way to accomplish the same thing is to use the named parameters
    of the ``FinServDatabricksRunNowOperator`` directly. Note that there is exactly
    ...
    endpoint. In this method, your code would look like this: ::
        ...
        notebook_run = FinServDatabricksRunNowOperator(
            job_id=job_id,
    ...
    Currently the named parameters that ``FinServDatabricksRunNowOperator`` supports are
        - ``job_id``
        - ``json``
        - ``notebook_params``
        - ``python_params``
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Creates a new ``FinServDatabricksRunNowOperator``.
        """
        super(FinServDatabricksRunNowOperator, self).__init__(**kwargs)
        self.json = json or {}
        self.databricks_conn_id = databricks_conn_id
        self.polling_period_seconds = polling_period_seconds
        self.databricks_retry_limit = databricks_retry_limit
