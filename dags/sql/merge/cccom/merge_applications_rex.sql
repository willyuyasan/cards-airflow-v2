insert into cccom_dw.fact_applications_rex(
application_id,
source_key,
click_id,
upload_file_id,
...
select
  ...
  'REX'
from (
  select * from cccom_dw.stg_applications_rex
  where state != 'DELETED'
  and application_id not in (
    select f.application_id from cccom_dw.fact_applications_rex f where f.origination_sys = 'REX')
) t;

update cccom_dw.fact_applications_rex
set status = 'DELETED',
load_date = sysdate
where origination_sys = 'REX'
  and application_id in (
  select s.application_id from cccom_dw.stg_applications_rex s
...
select
  ...
  'REX'
from (
  select * from cccom_dw.stg_declined_applications_rex
  where state != 'DELETED'
...
where origination_sys='REX'
  and application_id in (
  select s.declined_application_id from cccom_dw.stg_declined_applications_rex s
  where s.state = 'DELETED'
);


delete from cccom_dw.fact_applications_rex
where origination_sys='REX'
and submission_date >= add_months(date_trunc('month', sysdate), -3)
and is_declined = 0
and application_id not in (
    select application_id from cccom_dw.stg_applications_rex );