...
ot_details_staging_jar_task = {
    ...
    'parameters': [
        ...
        "START_DATE=" + (
                datetime.now() - (
            timedelta(days=int(int(Variable.get("DBX_SDK_Hourly_AMEX_OT_Lookback_Days")))))).strftime(
            "%Y-%m-%d"),
        "END_DATE=" + datetime.now().strftime("%Y-%m-%d"),
...
ot_raw_staging_jar_task = {
    ...
    'parameters': [
        ...
        "START_DATE=" + (
                datetime.now() - (
            timedelta(days=int(int(Variable.get("DBX_SDK_Hourly_AMEX_OT_Raw_Lookback_Days")))))).strftime(
            "%Y-%m-%d"),
        "END_DATE=" + datetime.now().strftime("%Y-%m-%d"),
...
[page_view_staging, page_metrics_staging, product_clicked_staging, product_viewed_staging, element_clicked_staging,
 ...
 form_submitted_staging, paidsearch_staging, cookies_staging, pzn_offers_received_staging, phone_system_call_staging,
