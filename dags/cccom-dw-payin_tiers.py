...
extract_payin_tiers = PythonOperator(
    ...
    op_kwargs={'extract_script': 'cccom/extract_payin_tiers.sql', 'key': 'payin_tiers.csv'},
    provide_context=True,
    dag=dag)
extract_card_assignments = PythonOperator(
    task_id='extract-cccom-payin_tier_card_assignments',
    python_callable=mysql_table_to_s3,
    op_kwargs={'extract_script': 'cccom/extract_payin_tier_card_assignments.sql', 'key': 'payin_tier_card_assignments.csv'},
    provide_context=True,
    dag=dag)
extract_website_assignments = PythonOperator(
    task_id='extract-cccom-payin_tier_website_assignments',
    python_callable=mysql_table_to_s3,
    op_kwargs={'extract_script': 'cccom/extract_payin_tier_website_assignments.sql', 'key': 'payin_tier_website_assignments.csv'},
    provide_context=True,
    dag=dag)
