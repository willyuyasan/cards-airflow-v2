...
resource "aws_acm_certificate" "airflow_cert" {
  count             = contains(local.airflow_deployments, var.workspace) ? 1 : 0
  domain_name       = var.workspace == "production" ? "*.airflow.${local.airflow_dns_zone_name}" : "*.airflow-${var.workspace}.${local.airflow_dns_zone_name}"
  validation_method = "DNS"
}

resource "aws_route53_record" "airflow_cert_validation" {
  count   = contains(local.airflow_deployments, var.workspace) ? 1 : 0
  name    = aws_acm_certificate.airflow_cert[0].domain_validation_options.*.resource_record_name[count.index]
  type    = aws_acm_certificate.airflow_cert[0].domain_validation_options.*.resource_record_type[count.index]
  zone_id = data.aws_route53_zone.main.zone_id
  records = [aws_acm_certificate.airflow_cert[0].domain_validation_options.*.resource_record_value[count.index]]
  ttl     = 60
}

resource "aws_acm_certificate_validation" "airflow_cert_verify" {
  count                   = contains(local.airflow_deployments, var.workspace) ? 1 : 0
  certificate_arn         = aws_acm_certificate.airflow_cert[0].arn
  validation_record_fqdns = aws_route53_record.airflow_cert_validation.*.fqdn
}
...
module "airflow" {
  ...
  acm_certificate_arn = aws_acm_certificate.airflow_cert[0].arn
