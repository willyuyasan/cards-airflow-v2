...
and ( (te.event_type_code = 'APP' and coalesce(ter.actual,0) <> 0 ) or
	  (te.event_type_code = 'REV' and t.tracking_id is not null and coalesce(ter.actual,0) <> 0 ) or
	  (te.event_type_code = 'APR' and coalesce(ter.actual,0) <> 0 and provider_file_id not like '1425453%') or 
	  (te.event_type_code = 'APR' and provider_file_id like '1425453%')
    )
