...
echo {{params.env}} {{params.pg_db_host}} {{params.pg_db_name}} {{params.pg_db_user}} {{params.file_suffix}}
...
psql -t -U {{params.pg_db_user}} -h {{params.pg_db_host}} -d {{params.pg_db_name}} -c "${SQL}" -o ${FILEPATH}${FILENAME}

# actual running vacuum
psql -h {{params.pg_db_host}} -d {{params.pg_db_name}} -U {{params.pg_db_user}} -f ${FILEPATH}${FILENAME} -o ${FILEPATH}${LOG_FILE}
