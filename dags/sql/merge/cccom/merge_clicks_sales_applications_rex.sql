...
delete
from cccom_dw.fact_clicks_sales_applications_rex
where click_date >= add_months(date_trunc('month', sysdate), -3);

insert into cccom_dw.fact_clicks_sales_applications_rex (
  click_id,
  click_date,
  product_key,
  affiliate_key,
...
left join (
  ...
  from cccom_dw.fact_sales_rex s
...
left join (
  ...
  from cccom_dw.fact_applications_rex
  where is_declined = 0
