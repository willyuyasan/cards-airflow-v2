...
where trans_id in (
  select s.trans_id from cccom_dw.stg_sale_trans s
);
...
-- orig      to_number(to_char(date(t.provider_process_date),'YYYYMMDD'),'99999999'),
utcview.provider_process_date_key,
-- orig     date_trunc('day', t.provider_process_date),
utcview.provider_process_date,
-- orig    t.provider_process_date,
utcview.provider_process_datetime,
t.merchant_name,
t.provider_id,
nvl(t.quantity,0),
t.provider_channel,
...
from (select s.* from cccom_dw.stg_sale_trans s where s.trans_id not in (select f.trans_id from cccom_dw.fact_sales f)) t
     join cccom_dw.stg_sale_trans_utc utcview on utcview.trans_id = t.trans_id and utcview.provider_process_datetime < '2019-02-01'
...
delete from cccom_dw.fact_sales
where provider_process_datetime >= (select min(s.provider_process_date) from cccom_dw.stg_sale_trans s)
and trans_id not in (select stg.trans_id from cccom_dw.stg_sale_trans stg)
-- cutover date logic
-- trans_type_key:110 - 102/make good
-- trans_type_key:117 - 120/Prepayment
and ((trans_type not in (102,120) and provider_process_date < '01-Feb-2019')
    or
    (trans_type_key in (102,120) and payout_date < '01-Mar-2019'))
  ;
