...
# by someone with either Account Owner or Embedded PE access within a teams account. Please see 
# riteshmshukla Install
locals {
  airflow_riteshmshukla_account       = "cards-data-developers"
  airflow_riteshmshukla_dns_zone_name = {
    testing     = "cards-nonprod.rvapps.io"
    development = "cards-nonprod.rvapps.io"
    staging     = "cards-nonprod.rvapps.io"
    qa          = "cards-nonprod.rvapps.io"
    production  = "cards.rvapps.io"
  }[var.workspace]
  airflow_riteshmshukla_deployments   = [
    "development",
    # "production"
  ]
...
## ACN Certificate
resource "aws_acm_certificate" "riteshmshukla_cert" {
  count             = contains(local.airflow_riteshmshukla_deployments, var.workspace) ? 1 : 0
  domain_name       = var.workspace == "production" ? "*.riteshmshukla.${local.airflow_riteshmshukla_dns_zone_name}" : "*.riteshmshukla-${var.workspace}.${local.airflow_riteshmshukla_dns_zone_name}"
  validation_method = "DNS"
}

resource "aws_route53_record" "riteshmshukla_cert_validation" {
  count   = contains(local.airflow_riteshmshukla_deployments, var.workspace) ? 1 : 0
  name    = aws_acm_certificate.riteshmshukla_cert[0].domain_validation_options.*.resource_record_name[count.index]
  type    = aws_acm_certificate.riteshmshukla_cert[0].domain_validation_options.*.resource_record_type[count.index]
  zone_id = data.aws_route53_zone.main.zone_id
  records = [aws_acm_certificate.riteshmshukla_cert[0].domain_validation_options.*.resource_record_value[count.index]]
  ttl     = 60
}

resource "aws_acm_certificate_validation" "riteshmshukla_cert_verify" {
  count                   = contains(local.airflow_riteshmshukla_deployments, var.workspace) ? 1 : 0
  certificate_arn         = aws_acm_certificate.riteshmshukla_cert[0].arn
  validation_record_fqdns = aws_route53_record.riteshmshukla_cert_validation.*.fqdn
}

# Actually spin up the Airflow implementation
module "airflow-riteshmshukla" {
  source  = "app.terraform.io/RVStandard/airflow/aws"
  version = "~> 1.0"

  enable           = contains(local.airflow_riteshmshukla_deployments, var.workspace)
  environment      = var.workspace
  project_name     = "riteshmshukla"
  owner            = local.airflow_riteshmshukla_owner
  team_name        = local.airflow_riteshmshukla_team_name
  image_version    = var.workspace == "production" ? local.airflow_riteshmshukla_prod_image_version : local.airflow_riteshmshukla_nonprod_image_version
  ...
  acm_certificate_arn = aws_acm_certificate.riteshmshukla_cert[0].arn
  dns_zone_name       = local.airflow_riteshmshukla_dns_zone_name
  ...
  # ssm__fernet_key = "arn:aws:ssm:us-east-1:${data.terraform_remote_state.core.outputs.account_id}:parameter/${local.airflow_riteshmshukla_team_name}-airflow-${var.workspace}/fernet_key"

  # If you have custom vpc tagging, use this to override which VPC to use for ECS placement
  vpc_tag_key_override = local.airflow_riteshmshukla_account
